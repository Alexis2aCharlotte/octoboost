...
export default function PublishPage() {
  ...
  async function handleTestConnection() {
    ...
    setTestSuccess(null);
    try {
      const action = (connection?.endpoint_url || connection?.type === "github") ? "test" : "verify-api";
      const res = await fetch(fetchUrl("/api/site-connection"), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ projectId: realProjectId, action }),
      });
      const data = await res.json();
      if (data.valid) {
        setConnection(data.connection);
        if (action === "verify-api") setTestSuccess("Connection detected — your site is fetching articles from OctoBoost.");
      } else {
        ...
        <div className="space-y-6">
          ...
          {apiKey && connection?.status !== "connected" && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 sm:p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-muted">Copy the key, integrate it on your site, visit your site, then test.</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
              ...
              {testError && <p className="mt-3 text-xs text-red-400">{testError}</p>}
              {testSuccess && <p className="mt-3 text-xs text-green-400">{testSuccess}</p>}
            </div>
          )}
