  ...
  if (analyzing) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-4 text-xs text-muted/60 sm:gap-8">
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {mainTab === "keywords" && (
        <>
          ...
          {kwTab === "keywords" && isFree && (
            <UpgradeCTA
              title="Unlock all keyword data"
              description={`${keywords.length} keywords discovered. Upgrade to see volumes, competition, SERP difficulty, and filter them.`}
            >
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {keywords.slice(0, 8).map((kw) => (
                        <tr key={kw.id} className="border-b border-border">
                          <td className="px-4 py-3"><div className="h-4 w-40 rounded bg-muted/20" /></td>
                          <td className="px-4 py-3"><div className="h-4 w-16 rounded bg-muted/10" /></td>
                          <td className="px-4 py-3"><div className="h-4 w-24 rounded bg-muted/10" /></td>
                          <td className="px-4 py-3"><div className="h-4 w-16 rounded bg-muted/10" /></td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {kwTab === "clusters" && isFree && (
            <UpgradeCTA
              title="Unlock all topic clusters"
              description={`${clusters.length} article topics discovered. Upgrade to see all clusters and generate articles for each.`}
            >
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {clusters.slice(0, 6).map((_, i) => (
                  <div key={i} className="rounded-xl border border-border bg-card p-5 space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="h-6 w-24 rounded bg-muted/10" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {kwTab === "clusters" && !isFree && <ClustersView clusters={clusters} />}
        </>
      )}
      ...
      {mainTab === "analysis" && analysisResult && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-4 sm:p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 space-y-1">
                ...
                >
                  <span className="truncate">{analysisResult.site.url}</span>
          ...
          <div>
            ...
            {isFree ? (
              <UpgradeCTA
                title="See your competitors"
                description={`We found ${analysisResult.competitors.length} competitors. Upgrade to see their names, URLs, and why they rank.`}
              >
                  ...
                  {analysisResult.competitors.map((comp) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <ExternalLink className="h-3.5 w-3.5 text-muted/20" />
                      </div>
                      <div className="mt-2 h-3 w-48 rounded bg-muted/10" />
...
function Tooltip({ text, children }: { text: string; children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      {pos && (
        <span
          className="fixed z-[9999] max-w-[220px] rounded-lg border border-border bg-[#0d1225] px-3 py-2 text-xs font-normal leading-relaxed text-muted shadow-lg"
